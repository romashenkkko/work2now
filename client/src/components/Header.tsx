    ...
    >
      <div className="container mx-auto px-4 py-[18px] flex items-center justify-between max-w-[1100px] gap-6">
        ...
        <div className="hidden md:flex items-center gap-3">
          {user ? (
            <>
              <Link to="/dashboard" className="btn-signup-php py-2.5 px-5 text-sm">
                {t("nav.hello", { name: user.name })}
              </Link>
              <button
                type="button"
                onClick={logout}
                className="btn-signin-php"
              >
                {t("nav.logout")}
              </button>
            </>
            ...
            <>
              <Link to="/login" className="btn-signin-php">
                {t("nav.signIn")}
              </Link>
              <button type="button" onClick={handleSignUp} className="btn-signup-php py-2.5 px-6 text-sm">
                {t("nav.signUp")}
              </button>
            </>
          )}
          <div ref={langDropdownRef} className={`lang-dropdown-wrap relative ${langOpen ? "lang-dropdown-open" : ""}`}>
            <button
              type="button"
              onClick={() => setLangOpen(!langOpen)}
              className="lang-btn-php flex items-center gap-1.5"
